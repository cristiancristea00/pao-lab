...
private:
    ...
    [[nodiscard]] auto EncryptSequence(std::vector<uint64_t> const & input)const noexcept -> std::vector<uint64_t>;
    [[nodiscard]] auto DecryptSequence(std::vector<uint64_t> const & input)const noexcept -> std::vector<uint64_t>;
    [[nodiscard]] auto EncryptDecryptSequence(std::vector<uint64_t> const & input) const -> std::vector<uint64_t>;
